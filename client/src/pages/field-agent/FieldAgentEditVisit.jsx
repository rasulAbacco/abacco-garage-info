...
const FieldAgentEditVisit = () => {
    ...
    const fetchExistingVisitPayload = async () => {
      try {
        ...
        if (data) {
          ...
          setReferredByNumber(
            data.referredByPhone || data.referredByNumber || "",
          );
          ...
          if (
            ...
            !baseCategories.includes(data.businessCategory)
          ) {
        ...
        setError(
          "Unable to process the structural lead components from the endpoint reference repository.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const removeContactRow = useCallback((index) => {
    setContacts((prev) => {
      if (prev[index].isPrimary && prev.length > 1) {
        alert(
          "Reassign structural explicit primary contact flag before processing row index erasure.",
        );
        return prev;
      }
  ...
  const removeEmailRow = useCallback(
    (index) => setEmails((prev) => prev.filter((_, i) => i !== index)),
    [],
  );
  ...
  const handleNewImageChange = (e) => {
    ...
    if (netCount > 10) {
      alert(
        "Exceeded maximum multi-image upload capacity structural boundary limitation (Max 10 Images total).",
      );
      return;
    }

    const validated = selectedFiles.filter((file) =>
  ...
  // Smart "back" navigation: prefer real browser history (this returns the
  // agent to wherever they actually came from — the My Visits list at its
  // current ?page=, or the Details page — instead of always forcing them
  // onto the Details page, which was creating a Details <-> Edit back/forward
  // loop since both pages' own back buttons pushed to each other).
  const handleGoBack = () => {
    if (
  ...
  const handleSubmit = async (e) => {
    ...
    if (!businessName.trim())
      return alert("Business Name title compilation tracking parameter error.");
    const validContacts = contacts.filter(
      (c) => c.name.trim() && c.phoneNumber.trim(),
    );
    if (validContacts.length === 0)
      return alert(
        "At least one named contact operational data registry trace mandatory.",
      );
    if (!contacts.some((c) => c.isPrimary))
      return alert(
        "One explicitly configured structural primary contact allocation required.",
      );
    if (!visitedDate)
      return alert("Visited Date is a required pipeline marker.");
    ...
    try {
      ...
      formData.append(
        "emails",
        JSON.stringify(emails.filter((em) => em.trim())),
      );
      ...
      Object.values(newDocPreviews).forEach((url) => {
      ...
      alert(
        err.response?.data?.message ||
          "Internal Exception Processing Platform Framework Pipeline Request Handler Execution Error.",
      );
    } finally {
    ...
    return (
      <div className="max-w-xl mx-auto my-16 text-center bg-white border border-slate-200 p-8 rounded-xl shadow-xs space-y-4">
        ...
        <h3 className="text-lg font-bold text-slate-900">
          Workspace Synchronization Halted
        </h3>
        <p className="text-xs text-slate-400">{error}</p>
        <button
          onClick={() => navigate("/field-agent-dashboard/my-visits")}
          className="px-4 py-2 text-xs font-bold uppercase bg-slate-950 text-white rounded-lg hover:bg-slate-800 transition-colors"
        >
          Return To Registry
        </button>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-2xs p-5 sm:p-6">
              <div className="flex items-center gap-2.5 mb-6 pb-3 border-b border-slate-100">
                ...
                <h2 className="text-base font-bold text-slate-900 tracking-tight">
                  Business Blueprint Modification
                </h2>
              </div>
                      ...
                      <div key={idx} className="flex items-center gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            value={email}
                            onChange={(e) =>
                              updateEmailRow(idx, e.target.value)
                            }
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-2xs p-5 sm:p-6">
              <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2.5">
                  ...
                  <h2 className="text-base font-bold text-slate-900 tracking-tight">
                    Personnel Directory Realignment
                  </h2>
                </div>
                <button
                  type="button"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-end relative">
                      ...
                      <div className="sm:col-span-3">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateContactRow(idx, "designation", e.target.value)
                          }
                      ...
                      <div className="sm:col-span-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateContactRow(idx, "phoneNumber", e.target.value)
                          }
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-2xs p-5 sm:p-6">
              <div className="flex items-center gap-2.5 mb-5 pb-3 border-b border-slate-100">
                ...
                <h2 className="text-base font-bold text-slate-900 tracking-tight">
                  Structured Files Matrix Re-indexing
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="border border-slate-200 p-4 rounded-xl bg-slate-50/50 space-y-3">
                  ...
                  {existingDocs.gstCertificate && (
                    <div className="flex items-center justify-between text-xs bg-white border border-slate-200 rounded-lg p-2">
                      <span className="truncate text-slate-600 font-semibold max-w-[70%]">
                        Active Asset Saved Profile
                      </span>
                      <a
                        href={existingDocs.gstCertificate}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex text-indigo-600 items-center gap-0.5 hover:underline text-[11px]"
                      >
                  ...
                  <input
                    ...
                    accept=".pdf,image/*"
                    onChange={(e) =>
                      handleNewDocFileChange(e, "gstCertificate")
                    }
                ...
                <div className="border border-slate-200 p-4 rounded-xl bg-slate-50/50 space-y-3">
                  ...
                  {existingDocs.quotationDoc && (
                    <div className="flex items-center justify-between text-xs bg-white border border-slate-200 rounded-lg p-2">
                      ...
                      <a
                        href={existingDocs.quotationDoc}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex text-indigo-600 items-center gap-0.5 hover:underline text-[11px]"
                      >
                ...
                <div className="border border-slate-200 p-4 rounded-xl bg-slate-50/50 space-y-3 sm:col-span-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                    Marketing Brochure Portfolio Collateral
                  </label>
                  {existingDocs.brochureDoc && (
                    <div className="flex items-center justify-between text-xs bg-white border border-slate-200 rounded-lg p-2">
                      <span className="truncate text-slate-600 font-semibold max-w-[80%]">
                        Active Asset Saved Profile
                      </span>
                      <a
                        href={existingDocs.brochureDoc}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex text-indigo-600 items-center gap-0.5 hover:underline text-[11px]"
                      >
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <option value="General Commercial">
                ...
                {businessCategory === "Others" && (
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 animate-fadeIn">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCustomBusinessCategory(e.target.value)
                      }
                      required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                    Lead Strategy Acquisition Source
                  </label>
                  <select
                    value={source}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                    Estimated Deal Value Size (₹)
                  </label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-500 text-xs font-bold">
                      ₹
                    </span>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-slate-400" /> Visited
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                      <Calendar className="w-3 h-3 text-slate-400" /> Target
                      Date
                    </label>
                    <input
                      type="date"
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-2xs p-5 sm:p-6">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {AVAILABLE_PRODUCTS.map((prod) => (
                  <label
                    key={prod}
                    className="flex items-center gap-2 border border-slate-100 bg-slate-50/50 p-2 rounded-xl text-xs font-medium cursor-pointer select-none text-slate-800 hover:bg-slate-50 transition-colors"
                  >
                    <input
                      type="checkbox"
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">
                    Corporate Card Front Face
                  </label>
                  {existingDocs.businessCardFront && (
                    <p className="text-[10px] text-emerald-600 font-bold mb-1 flex items-center gap-0.5">
                      <ShieldCheck className="w-3 h-3" /> Front card profile
                      asset saved on backend store partition.
                    </p>
                  )}
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) =>
                      handleNewDocFileChange(e, "businessCardFront")
                    }
                  ...
                  {newDocPreviews.businessCardFront && (
                    <div className="mt-2 relative aspect-[3.5/2] w-full rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
                      <img
                        src={newDocPreviews.businessCardFront}
                        alt="Identity Front Staged Asset Preview"
                        className="w-full h-full object-cover"
                      />
                      <button
                        type="button"
                        onClick={() =>
                          handleRemoveNewDocFile("businessCardFront")
                        }
                        className="absolute top-1.5 right-1.5 p-1 bg-black/60 rounded-full text-white hover:bg-rose-600"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">
                    Corporate Card Reverse Face
                  </label>
                  {existingDocs.businessCardBack && (
                    <p className="text-[10px] text-emerald-600 font-bold mb-1 flex items-center gap-0.5">
                      <ShieldCheck className="w-3 h-3" /> Back card profile
                      asset saved on backend store partition.
                    </p>
                  )}
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      handleNewDocFileChange(e, "businessCardBack")
                    }
                    className="text-xs w-full block text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-bold file:bg-slate-100 file:text-slate-800 hover:file:bg-slate-200 cursor-pointer"
                  />
                  {newDocPreviews.businessCardBack && (
                    <div className="mt-2 relative aspect-[3.5/2] w-full rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
                      <img
                        src={newDocPreviews.businessCardBack}
                        alt="Identity Reverse Staged Asset Preview"
                        className="w-full h-full object-cover"
                      />
                      <button
                        type="button"
                        onClick={() =>
                          handleRemoveNewDocFile("businessCardBack")
                        }
                        className="absolute top-1.5 right-1.5 p-1 bg-black/60 rounded-full text-white hover:bg-rose-600"
                      >
                        <X className="w-3 h-3" />
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-2xs p-5 sm:p-6">
              ...
              <p className="text-xs text-slate-400 mb-4">
                Re-index, clear or allocate up to 10 visual operational
                verification storefront images total.
              </p>
                  ...
                  <div className="grid grid-cols-4 gap-2 border p-2 rounded-xl bg-slate-50/50">
                    {existingImages.map((img) => {
                      ...
                      return (
                        <div
                          key={img.id}
                          className={`relative aspect-square border bg-white rounded-lg overflow-hidden transition-opacity ${isDeleted ? "opacity-30 border-rose-300" : "border-slate-200"}`}
                        >
                          <img
                            src={img.imageUrl}
                            alt="Server verification asset frame block"
                            className="w-full h-full object-cover"
                          />
                          {!isDeleted ? (
                            <button
                              type="button"
                              onClick={() =>
                                handleMarkExistingImageDeleted(img.id)
                              }
                            ...
                            <span className="absolute inset-0 flex items-center justify-center text-[8px] font-black text-rose-600 uppercase bg-rose-50/70 select-none tracking-tighter">
                              Will Purge
                            </span>
                          )}
                        </div>
              ...
              <label className="group flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-slate-300 hover:border-slate-900 rounded-xl transition-all duration-150 cursor-pointer bg-slate-50/40 hover:bg-slate-50">
                <div className="flex flex-col items-center justify-center pt-3 pb-3 text-center px-4">
                  ...
                  <p className="text-xs text-slate-600 font-semibold">
                    Stage new images onto upload stream pipeline
                  </p>
                </div>
                <input
                  type="file"
                ...
                <div className="grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-slate-100">
                  {newImagePreviews.map((url, idx) => (
                    ...
                    >
                      <img
                        src={url}
                        alt={`Staged Photo Matrix Track ${idx + 1}`}
                        className="w-full h-full object-cover"
                      />
                      <button
                        type="button"
      ...
      <footer className="fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 py-3.5 px-4 sm:px-6 md:px-8 shadow-md z-40 backdrop-blur-md bg-white/95">
        <div className="max-w-7xl mx-auto flex items-center justify-end gap-3">
          ...
          >
            ...
            {isSubmitting
              ? "Committing Structure Revisions..."
              : "Commit Lead Changes"}
          </button>
        </div>
      </footer>
