...
const FieldAgentDashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardPayload = async () => {
            if (!user.id) {
                setError("Security session validation identity profile matrix map context missing.");
                setLoading(false);
                return;
            }
            try {
                ...
                if (followUpsRes.status === "fulfilled") {
                    setFollowUps(Array.isArray(followUpsRes.value.data) ? followUpsRes.value.data : followUpsRes.value.data?.followUps || []);
                }
                if (visitsRes.status === "fulfilled") {
                    const rawVisits = Array.isArray(visitsRes.value.data) ? visitsRes.value.data : visitsRes.value.data?.visits || [];
                    setLatestVisits(rawVisits);
                }
            } catch (err) {
                console.error("Dashboard orchestrator extraction barrier failure: ", err);
                setError("Failed to compile synchronized runtime telemetry matrices from data lake pipelines.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const calculatedMetrics = useMemo(() => {
        const counts = { total: latestVisits.length, pending: 0, interested: 0, customer: 0, overDue: 0 };
        ...
        latestVisits.forEach(v => {
            ...
            else if (statusKey === "CUSTOMER") counts.customer++;

            if (v.followUpDate && new Date(v.followUpDate).setHours(0, 0, 0, 0) < todayTimestamp && statusKey !== "CUSTOMER" && statusKey !== "NOT_INTERESTED") {
                counts.overDue++;
            }
        });

        const successRate = counts.total ? Math.round((counts.customer / counts.total) * 100) : 0;
        const pendingPct = counts.total ? Math.round((counts.pending / counts.total) * 100) : 0;
        const interestedPct = counts.total ? Math.round((counts.interested / counts.total) * 100) : 0;
        const closedPct = counts.total ? Math.round((counts.customer / counts.total) * 100) : 0;

        // Calculate Average Visits Per Day Based on Unique Days Inhabited
        const uniqueDays = new Set(latestVisits.map(v => v.createdAt ? new Date(v.createdAt).toISOString().split("T")[0] : null).filter(Boolean));
        const avgVisitsPerDay = uniqueDays.size ? (counts.total / uniqueDays.size).toFixed(1) : counts.total;

        return { ...counts, successRate, pendingPct, interestedPct, closedPct, avgVisitsPerDay };
    }, [latestVisits]);
    ...
    const chartMatrices = useMemo(() => {
        ...
        // 3. Status allocation
        const statusMap = { PENDING: 0, FOLLOW_UP: 0, INTERESTED: 0, CUSTOMER: 0, NOT_INTERESTED: 0 };

        // 4. Marketing segment mapping allocation
        const typeMap = {};

        const currentTimestamp = new Date();
        const clearWeekTimestamp = new Date(currentTimestamp.setDate(currentTimestamp.getDate() - currentTimestamp.getDay()));
        ...
        const statusChartData = Object.keys(statusMap).map(k => ({ name: k, value: statusMap[k] })).filter(d => d.value > 0);
        const typeChartData = Object.keys(typeMap).map(k => ({ name: k, value: typeMap[k] })).filter(d => d.value > 0);
        const monthlyChartData = Object.keys(monthlyMap).map(k => ({ name: k, Visits: monthlyMap[k] }));

        return {
            weekly: weeklyData,
            status: statusChartData,
            marketing: typeChartData,
            monthly: monthlyChartData.length ? monthlyChartData : [{ name: "Current Month", Visits: latestVisits.length }]
        };
    }, [latestVisits]);

    // Colors Arrays Mapping Lists Specifications
    const COLORS = ["#0f172a", "#2563eb", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#64748b"];
            ...
            <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-md px-4 py-3 sm:px-6 md:px-8 shadow-xs">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-3.5">
                    ...
                    <div className="flex items-center gap-3 order-1 md:order-2 self-end md:self-auto text-xs bg-slate-100/80 px-3 py-1.5 rounded-lg border border-slate-200">
                        ...
                        <span className="font-bold text-slate-400 uppercase tracking-widest text-[9px]">Identity Scope ID:</span>
            ...
            <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 md:px-8 pb-32 space-y-6">
                ...
                <section className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-400 font-medium max-w-xl">
                            Welcome to your workstation panel grid terminal. Tracking and processing leads within the <span className="font-bold text-slate-700">{user.marketingType || "GENERAL MARKETING"}</span> vertical network layout structure map.
                        </p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-rose-50 border border-rose-200 text-rose-800 p-5 rounded-xl flex items-start gap-3 shadow-xs">
                        ...
                        <div>
                            <h4 className="font-extrabold text-sm uppercase tracking-wider">System Integration Blocked Exception</h4>
                    ...
                    <>
                        ...
                        <section className="grid grid-cols-2 lg:grid-cols-6 gap-3.5 sm:gap-4">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Today's Visits</span>
                                ...
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">Logs captured today</span>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Pipeline</span>
                                <span className="text-2xl font-black text-slate-900 block mt-1 tabular-nums">
                                    {summaryData?.totalVisitsCount ?? calculatedMetrics.total}
                                </span>
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">Lifetime lead records</span>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">This Month</span>
                                <span className="text-2xl font-black text-slate-900 block mt-1 tabular-nums">
                                    {summaryData?.thisMonthVisitsCount ?? latestVisits.filter(v => v.createdAt && new Date(v.createdAt).getMonth() === new Date().getMonth()).length}
                                </span>
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">Current monthly target</span>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pending Follows</span>
                                <span className="text-2xl font-black text-slate-900 block mt-1 tabular-nums">
                                    {summaryData?.pendingFollowUpsCount ?? calculatedMetrics.pending}
                                </span>
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">Awaiting re-engagement</span>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Interested Leads</span>
                                <span className="text-2xl font-black text-slate-900 block mt-1 tabular-nums">
                                    {summaryData?.interestedLeadsCount ?? calculatedMetrics.interested}
                                </span>
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">High potential prospects</span>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs hover:shadow-md transition-all group relative">
                                ...
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Closed Deals</span>
                                ...
                                <span className="text-[9px] text-slate-400 font-medium mt-1 block">Successfully converted accounts</span>
                            </div>
                        </section>
                        ...
                        <section className="bg-white rounded-xl border border-slate-200 shadow-xs p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            >
                                <div>
                                    <span className="block text-xs font-bold uppercase tracking-wider text-inherit">Add New Visit</span>
                                    <span className="text-[10px] text-slate-400 group-hover:text-slate-300 font-medium">Record business lead</span>
                                </div>
                            ...
                            >
                                <div>
                                    <span className="block text-xs font-bold uppercase tracking-wider text-inherit">My Visits</span>
                                    <span className="text-[10px] text-slate-400 group-hover:text-slate-300 font-medium">View tracked registry</span>
                                </div>
                                <Layers className="w-4 h-4 opacity-60 group-hover:opacity-100 shrink-0" />
                            </button>

                            <button
                                onClick={() => navigate("/field-agent-dashboard/my-visits?filter=today")}
                                className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50 hover:bg-slate-950 hover:text-white transition-all text-left group cursor-pointer"
                            >
                                <div>
                                    <span className="block text-xs font-bold uppercase tracking-wider text-inherit">Today's Follow Ups</span>
                                    <span className="text-[10px] text-slate-400 group-hover:text-slate-300 font-medium">Triage client pipelines</span>
                                </div>
                            ...
                            >
                                <div>
                                    <span className="block text-xs font-bold uppercase tracking-wider text-inherit">My Profile</span>
                                    <span className="text-[10px] text-slate-400 group-hover:text-slate-300 font-medium">Manage credentials</span>
                                </div>
                            ...
                            <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {calculatedMetrics.overDue > 0 && (
                                    <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 flex items-center gap-3.5 shadow-2xs animate-fadeIn md:col-span-1">
                                        ...
                                        <div>
                                            <span className="block text-[10px] font-bold text-rose-800 uppercase tracking-wider">Overdue Alert Warning</span>
                                            <p className="text-xs font-medium text-rose-700 mt-0.5">
                                                You have <span className="font-extrabold">{calculatedMetrics.overDue}</span> pipeline follow-up interactions that require urgent execution.
                                            </p>
                                        </div>
                                    </div>
                                )}
                                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3.5 shadow-2xs animate-fadeIn md:col-span-2">
                                    ...
                                    <div>
                                        <span className="block text-[10px] font-bold text-amber-800 uppercase tracking-wider">Scheduler Event Matrix</span>
                        ...
                        <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 lg:col-span-7 flex flex-col justify-between min-h-[360px]">
                                <div className="mb-4">
                                    <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                                        <BarChart3 className="w-4 h-4 text-indigo-600" /> Visits Recorded This Week
                                    </h3>
                                </div>
                                <div className="w-full h-64 text-xs font-semibold">
                                    {latestVisits.length > 0 ? (
                                        <ResponsiveContainer width="100%" h="100%">
                                            <BarChart data={chartMatrices.weekly} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
                                                ...
                                                <Tooltip cursor={{ fill: '#f8fafc' }} contentStyle={{ background: '#0f172a', color: '#fff', borderRadius: '8px' }} />
                                                <Bar dataKey="Visits" fill="#0f172a" radius={[4, 4, 0, 0]} barSize={28} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center bg-slate-50 border border-dashed border-slate-200 rounded-xl text-slate-400 text-xs">No current week timeline data stream active</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 lg:col-span-5 flex flex-col justify-between min-h-[360px]">
                                <div className="mb-4">
                                    <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                                        <Layers className="w-4 h-4 text-indigo-600" /> Lead Pipeline Allocation
                                    </h3>
                                </div>
                                <div className="w-full h-56 text-xs flex items-center justify-center relative">
                                    {chartMatrices.status.length > 0 ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center bg-slate-50 border border-dashed border-slate-200 rounded-xl text-slate-400 text-xs">No pipeline allocations recorded</div>
                                    )}
                                </div>
                                <div className="flex flex-wrap items-center justify-center gap-3 text-[10px] uppercase font-bold tracking-wider text-slate-500 pt-2 border-t border-slate-50">
                                    {chartMatrices.status.map((entry, idx) => (
                                        <span key={entry.name} className="flex items-center gap-1">
                                            <span className="h-2 w-2 rounded-full block" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                        ...
                        <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-3">
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Conversion Success Rate</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-slate-900 tabular-nums">{calculatedMetrics.successRate}%</span>
                                    <span className="text-xs font-semibold text-emerald-600">Total conversion mapping</span>
                                </div>
                                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                    <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${calculatedMetrics.successRate}%` }} />
                                </div>
                            </div>

                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-3">
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Interested Vector Velocity</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-slate-900 tabular-nums">{calculatedMetrics.interestedPct}%</span>
                                    <span className="text-xs font-semibold text-blue-600">Hot interaction state</span>
                                </div>
                                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                    <div className="bg-blue-500 h-full rounded-full" style={{ width: `${calculatedMetrics.interestedPct}%` }} />
                                </div>
                            </div>

                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-3">
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Awaiting Nurture Velocity</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-slate-900 tabular-nums">{calculatedMetrics.pendingPct}%</span>
                                    <span className="text-xs font-semibold text-amber-600">Pending initial audit</span>
                                </div>
                                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                    <div className="bg-amber-500 h-full rounded-full" style={{ width: `${calculatedMetrics.pendingPct}%` }} />
                                </div>
                            </div>

                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-3">
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Daily Operations Velocity</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-2xl font-black text-slate-900 tabular-nums">{calculatedMetrics.avgVisitsPerDay}</span>
                                    <span className="text-xs font-semibold text-slate-500">Avg visits per active day</span>
                                </div>
                                <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                    <div className="bg-slate-900 h-full rounded-full" style={{ width: `${Math.min(Number(calculatedMetrics.avgVisitsPerDay) * 10, 100)}%` }} />
                                </div>
                            </div>
                        ...
                        <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 lg:col-span-5 space-y-4">
                                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                    <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                                        <Clock className="w-4 h-4 text-slate-900" /> Today's Scheduled Targets
                                    </h3>
                                ...
                                {followUps.length === 0 ? (
                                    <div className="text-center py-8 bg-slate-50 border border-dashed border-slate-200 rounded-xl space-y-2">
                                        <p className="text-xs text-slate-400 font-medium">No target follow-ups assigned on today's matrix.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {followUps.slice(0, 5).map((follow) => (
                                            <div key={follow.id} className="border border-slate-200 hover:border-slate-300 p-3.5 rounded-xl transition-all space-y-2.5 bg-slate-50/40">
                                                <div className="flex justify-between items-start gap-2">
                                                    <div>
                                                        <h4 className="text-xs font-bold text-slate-900 line-clamp-1">{follow.title || "Unnamed Prospect Enterprise"}</h4>
                                                        <span className="text-[10px] text-slate-400 font-medium block mt-0.5 flex items-center gap-1">
                                                            <User className="w-3 h-3" /> Contact: {follow.contactPerson || "N/A"}
                                                        </span>
                                                    </div>
                                                    <span className={`text-[9px] font-black tracking-wider uppercase border px-1.5 py-0.5 rounded-sm shrink-0 bg-white ${getStatusBadgeStyle(follow.status)}`}>
                                                ...
                                                <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                                                    <span className="text-[10px] font-bold tracking-tight tabular-nums text-slate-500 flex items-center gap-1">
                                                        <Phone className="w-3 h-3 text-slate-400" /> {follow.phoneNumber || "No digits"}
                                                    </span>
                                                    ...
                                                    <div className="flex items-center gap-1.5 shrink-0">
                                                        ...
                                                        <button
                                                            onClick={() => navigate(`/field-agent-dashboard/visit/${follow.id}`)}
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 lg:col-span-7 space-y-4">
                                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                    <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                                        <Layers className="w-4 h-4 text-slate-900" /> Recent Visits Feed Channel
                                    </h3>
                                    <button
                                        onClick={() => navigate("/field-agent-dashboard/my-visits")}
                                ...
                                {filteredRecentVisits.length === 0 ? (
                                    <div className="text-center py-12 bg-slate-50 border border-dashed border-slate-200 rounded-xl space-y-4 animate-fadeIn">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold text-slate-800">No visits matching queries yet.</p>
                                            <p className="text-[11px] text-slate-400 max-w-xs mx-auto">Your field matrix mapping parameters register holds no historical verification instances yet.</p>
                                        </div>
                                        <button
                                            onClick={() => navigate("/field-agent-dashboard/add-visit")}
                                    ...
                                    <div className="divide-y divide-slate-100">
                                        {filteredRecentVisits.map((visit) => {
                                            ...
                                            return (
                                                <div key={visit.id} className="py-3.5 first:pt-0 last:pb-0 flex items-center justify-between gap-3 group">
                                                    <div className="flex items-center gap-3 min-w-0">
                                                        {/* Thumbnail */}
                                                        <div className="h-11 w-11 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden shrink-0">
                                                            {sampleImage ? (
                                                                <img src={sampleImage} alt="Storefront asset snapshot" className="h-full w-full object-cover transform duration-200 group-hover:scale-105" />
                                                            ) : (
                                                        ...
                                                        <div className="min-w-0 space-y-0.5">
                                                            <h4 className="text-xs font-bold text-slate-900 truncate tracking-tight">{visit.title || "Unnamed Lead Instance"}</h4>
                                                            <div className="flex items-center gap-2 text-[10px] text-slate-400 font-semibold uppercase tracking-wider">
                                                                <span className="text-slate-700 bg-slate-100 px-1 rounded-sm text-[9px] font-black">{visit.marketingType || "GENERAL"}</span>
                                                                <span>•</span>
                                                                <span className="tabular-nums">{visit.createdAt ? new Date(visit.createdAt).toLocaleDateString("en-GB", { day: '2-digit', month: 'short' }) : "N/A"}</span>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div className="flex items-center gap-3 shrink-0">
                                                        <span className={`text-[9px] font-black tracking-widest border px-1.5 py-0.5 rounded-sm uppercase bg-white ${getStatusBadgeStyle(visit.status)}`}>
