...
const FieldAgentVisitDetails = () => {
  ...
  useEffect(() => {
    const fetchVisitDetails = async () => {
      try {
        ...
        setError(
          "Failed to construct synchronized pipeline data matching this lead token.",
        );
      } finally {
        setLoading(false);
      }
  ...
  // Smart "back" navigation: prefer real browser history (this preserves
  // whatever ?page=/?perPage= the My Visits list was on when the agent
  // opened this card) and only fall back to a plain list URL when there's
  // no prior in-app history to return to (e.g. this page was opened
  ...
  const handleDelete = async () => {
    try {
      ...
      alert(
        "Business lead history parameters wiped successfully from cloud registry.",
      );
      navigate("/field-agent-dashboard/my-visits");
    } catch (err) {
      console.error(err);
      alert(
        err.response?.data?.message ||
          "Internal transaction depletion tracking exception.",
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  const getPriorityStyle = (p) => {
    ...
    if (pr === "LOW")
      return "text-slate-500 font-bold bg-slate-100 border-slate-200";
    return "text-amber-600 font-bold bg-amber-50 border-amber-100";
  };

  // Safe Image Array Extraction Helper
  const parsedContacts =
    typeof visit?.contacts === "string"
      ? JSON.parse(visit.contacts)
      : visit?.contacts || [];
  const parsedEmails =
    typeof visit?.emails === "string"
      ? JSON.parse(visit.emails)
      : visit?.emails || [];
  const parsedProducts =
    typeof visit?.interestedProducts === "string"
      ? JSON.parse(visit.interestedProducts)
    ...
    return (
      <div className="max-w-xl mx-auto my-16 text-center bg-white border border-slate-200 p-8 rounded-xl shadow-xs space-y-4">
        ...
        <h3 className="text-lg font-bold text-slate-900">
          Resource Retrieval Blocked
        </h3>
        <p className="text-xs text-slate-400">
          {error ||
            "The requested visit ledger reference does not map to any structured asset on this cloud partition."}
        </p>
        <button
  ...
  const docFilesList = [
    {
      key: "businessCardFront",
      label: "Business Card Front",
      url: visit.businessCardFront,
    },
    {
      key: "businessCardBack",
      label: "Business Card Back",
      url: visit.businessCardBack,
    },
    {
      key: "gstCertificate",
      label: "GST Certificate",
      url: visit.gstCertificate,
    },
    {
      key: "quotationDoc",
      label: "Provisional Quotation",
      url: visit.quotationDoc,
    },
    {
      key: "brochureDoc",
      label: "Material Brochure Portfolio",
      url: visit.brochureDoc,
    },
  ].filter((d) => d.url);
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-md px-4 py-4 sm:px-6 md:px-8 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span
                  className={`text-[9px] font-black uppercase tracking-widest border px-2 py-0.5 rounded-sm bg-white ${getStatusBadgeStyle(visit.status)}`}
                >
              ...
              <p className="text-[10px] font-semibold text-slate-400 flex items-center gap-1.5 mt-0.5">
                <Clock className="w-3 h-3" /> Mapped parameters:{" "}
                {formatDateString(visit.createdAt)} • Synchronized check:{" "}
                {formatDateString(visit.updatedAt || visit.createdAt)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
            ...
            <button
              onClick={() =>
                navigate(`/field-agent-dashboard/edit-visit/${visit.id}`)
              }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
                <div className="bg-slate-50/60 p-3 rounded-lg border border-slate-100">
                  <span className="block font-bold text-slate-400 uppercase tracking-wider text-[9px] mb-0.5">
                    Category Node
                  </span>
                  <span className="font-bold text-slate-800">
                    {visit.businessCategory || "Unspecified Category"}
                  </span>
                </div>
                <div className="bg-slate-50/60 p-3 rounded-lg border border-slate-100">
                  <span className="block font-bold text-slate-400 uppercase tracking-wider text-[9px] mb-0.5">
                ...
                <div className="bg-slate-50/60 p-3 rounded-lg border border-slate-100">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-md text-[10px] inline-block ${getPriorityStyle(visit.priority)}`}
                  >
                ...
                <div className="bg-slate-50/60 p-3 rounded-lg border border-slate-100">
                  <span className="block font-bold text-slate-400 uppercase tracking-wider text-[9px] mb-0.5">
                    Dialogue Result Metric
                  </span>
                  <span className="font-bold text-slate-800">
                    {visit.meetingResult || "Discussed"}
                  </span>
                </div>
                <div className="bg-slate-50/60 p-3 rounded-lg border border-slate-100 col-span-2 sm:col-span-2">
                  ...
                  <span className="font-black text-slate-900 text-sm tracking-tight tabular-nums">
                    {visit.leadValue
                      ? `₹ ${Number(visit.leadValue).toLocaleString("en-IN")}`
                      : "₹ 0.00"}
                  </span>
                </div>
              </div>
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              <div className="flex items-center gap-2.5 mb-5 pb-3 border-b border-slate-100">
                ...
                <h2 className="text-base font-bold text-slate-900 tracking-tight">
                  Personnel Directory Matrix
                </h2>
              </div>

              {parsedContacts.length === 0 ? (
                <p className="text-xs text-slate-400 italic py-2">
                  No structured individual contact parameters indexed against
                  this account.
                </p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {parsedContacts.map((c, index) => (
                    <div
                      key={index}
                      className={`p-4 rounded-xl border border-slate-200 bg-slate-50/40 space-y-3 relative ${c.isPrimary ? "border-slate-950 bg-white shadow-2xs" : ""}`}
                    >
                      {c.isPrimary && (
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-slate-900 truncate pr-16">
                          {c.name || "Unnamed Representative"}
                        </h4>
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                          {c.designation || "Executive Stakeholder"}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs border-t border-slate-100 pt-2.5 gap-2 font-mono">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <a
                            href={`https://wa.me/91${c.phoneNumber?.replace(/[^0-9]/g, "")}`}
                            target="_blank"
                            rel="noreferrer"
              ...
              <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
                ...
                <div className="space-y-2">
                  {parsedEmails.map((email, eIdx) => (
                    <div
                      key={eIdx}
                      className="flex items-center justify-between text-xs bg-slate-50 px-3 py-2 rounded-lg border border-slate-200 font-mono"
                    >
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              <div className="flex items-center gap-2.5 mb-4 pb-3 border-b border-slate-100">
                ...
                <h2 className="text-base font-bold text-slate-900 tracking-tight">
                  Interaction Discussion Summary
                </h2>
              </div>
              <div className="bg-slate-50 border border-slate-200/60 p-4 rounded-xl min-h-[100px]">
                <p className="text-xs text-slate-700 font-medium leading-relaxed whitespace-pre-wrap">
                  {visit.discussionSummary ||
                    visit.notes ||
                    "No strategic summary dialogue notes captured on site parameter indexes matching current workflow shift logs."}
                </p>
              </div>
            </section>
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              ...
              {standardImages.length === 0 ? (
                <div className="text-center py-6 bg-slate-50/50 border border-dashed border-slate-200 rounded-xl text-slate-400 text-xs">
                  No workspace layout verification pictures logged in ambient
                  directory vaults.
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {standardImages.map((url, idx) => (
                    ...
                    >
                      <img
                        src={url}
                        alt={`Ambient audit storefront track piece index ${idx + 1}`}
                        className="w-full h-full object-cover transform duration-200 group-hover:scale-105"
                      />
          ...
          <div className="space-y-6">
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              <div className="flex items-center gap-2.5 mb-5 pb-3 border-b border-slate-100">
                ...
                <h2 className="text-base font-bold text-slate-900 tracking-tight">
                  Follow Up Matrix Configuration
                </h2>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-slate-950 text-white rounded-xl p-4 flex items-center justify-between text-xs shadow-inner">
                  <span className="font-bold text-slate-400 uppercase tracking-widest text-[9px] flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-slate-400" /> Next Matrix
                    Check
                  </span>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="border border-slate-200 p-2.5 rounded-lg bg-slate-50/50">
                    ...
                    <span className="font-bold text-slate-800">
                      {formatDateString(visit.visitedDate)}
                    </span>
                  </div>
                  <div className="border border-slate-200 p-2.5 rounded-lg bg-slate-50/50">
                    <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">
                    ...
                    <span className="font-bold text-slate-800">
                      {visit.nextFollowUpMode || "Phone Call"}
                    </span>
                  </div>
              ...
              <div className="space-y-3.5 text-xs text-slate-600">
                <p className="leading-relaxed font-semibold text-slate-800">
                  {[visit.address, visit.city, visit.district, visit.state]
                    .filter(Boolean)
                    .join(", ") ||
                    "No physical street address components configured."}
                </p>
                ...
                >
                  <ExternalLink className="w-3.5 h-3.5" /> Navigate via Google
                  Maps
                </a>
              </div>
            </section>
            ...
            <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs">
              ...
              {parsedProducts.length === 0 ? (
                <p className="text-xs text-slate-400 italic">
                  No specific system core product indices cross-checked during
                  verification shift parameters.
                </p>
              ) : (
                <div className="flex flex-wrap gap-1.5">
                  {parsedProducts.map((p, pIdx) => (
                    <span
                      key={pIdx}
                      className="bg-slate-100 border border-slate-200 font-bold text-slate-800 px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wide"
                    >
                      {p}
                    </span>
                  ))}
              ...
              <section className="bg-white rounded-xl border border-slate-200 p-5 sm:p-6 shadow-2xs text-xs space-y-3 bg-slate-50/20">
                ...
                <div className="space-y-1.5 font-medium text-slate-600">
                  <p className="font-bold text-slate-900 text-sm">
                    {visit.employee.name || "Field Officer Registry Node"}
                  </p>
        ...
        <div className="fixed inset-0 bg-slate-950/95 z-50 flex flex-col items-center justify-center p-4 backdrop-blur-xs animate-fadeIn">
          ...
          <div className="absolute top-4 inset-x-4 flex items-center justify-between text-white z-50">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() =>
                  setLightbox({ show: false, images: [], index: 0, scale: 1 })
                }
                className="p-2 bg-white/20 hover:bg-rose-600 rounded-full cursor-pointer transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl max-w-sm w-full p-6 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                This transaction performs immediate deletion of the account lead
                from cloud data tables. Stored personal matrix arrays,
                documentation references, and follow-up timeline nodes will be
      ...
      {showFollowUpModal && (
        <FollowUpModal
          ...
          onSaved={(followUps, savedFollowUp) => {
            ...
            // followUpController.createFollowUp): only these
            // three follow-up statuses promote the visit's
            // overall status.
            ...
            setVisit((prev) => {
              ...
              return {
                ...
                latestFollowUpRemark:
                  latest?.remark || prev.latestFollowUpRemark,
                latestFollowUpStatus:
                  latest?.status || prev.latestFollowUpStatus,
                latestFollowUpPriority:
                  latest?.priority || prev.latestFollowUpPriority,
              };
            });
          }}
